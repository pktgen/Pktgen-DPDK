...
char *
pktgen_ether_hdr_ctor(port_info_t *info, pkt_seq_t *pkt, struct __ether_hdr *eth)
{
	uint32_t flags;
	uint16_t vlan_id;
	/* src and dest addr */
	__ether_addr_copy(&pkt->eth_src_addr, &eth->s_addr);
	__ether_addr_copy(&pkt->eth_dst_addr, &eth->d_addr);

	flags = rte_atomic32_read(&info->port_flags);
	if (flags & SEND_VLAN_ID) {
		/* vlan ethernet header */
		eth->ether_type = htons(__ETHER_TYPE_VLAN);

		/* only set the TCI field for now; don't bother with PCP/DEI */
		struct __vlan_hdr *__vlan_hdr = (struct __vlan_hdr *)(eth + 1);
		vlan_id = (pkt->vlanid | (pkt->cos << 13));
		__vlan_hdr->vlan_tci = htons(vlan_id);
		__vlan_hdr->eth_proto = htons(pkt->ethType);

		/* adjust header size for VLAN tag */
		pkt->ether_hdr_size = sizeof(struct __ether_hdr) +
			sizeof(struct __vlan_hdr);

		return (char *)(__vlan_hdr + 1);
		...
		qinq_hdr->vlan_tpid = htons(__ETHER_TYPE_VLAN);
