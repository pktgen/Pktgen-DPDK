...
typedef struct range_info_s {
	/**< Source IP increment */
	union { uint32_t src_ip_inc; uint8_t src_ipv6_inc[PG_IN6ADDRSZ]; };
	/**< Destination increment IP address */
	union { uint32_t dst_ip_inc; uint8_t dst_ipv6_inc[PG_IN6ADDRSZ]; };
	...
	/**< TTL inc */
	union { uint8_t ttl_inc; uint8_t hop_limits_inc; };

	uint8_t pad0[3];

	/**< Source starting IP address */
	union { uint32_t src_ip; uint8_t src_ipv6[PG_IN6ADDRSZ]; };
	/**< Source IP minimum */
	union {	uint32_t src_ip_min; uint8_t src_ipv6_min[PG_IN6ADDRSZ]; };
	/**< Source IP maximum */
	union { uint32_t src_ip_max; uint8_t src_ipv6_max[PG_IN6ADDRSZ]; };
	/**< Destination starting IP address */
	union { uint32_t dst_ip; uint8_t dst_ipv6[PG_IN6ADDRSZ]; };
	/**< Destination minimum IP address */
	union { uint32_t dst_ip_min; uint8_t dst_ipv6_min[PG_IN6ADDRSZ]; };
	/**< Destination maximum IP address */
	union { uint32_t dst_ip_max; uint8_t dst_ipv6_max[PG_IN6ADDRSZ]; };
	...
	/**< tos val starting */
	union { uint8_t tos; uint8_t traffic_class; };
	/**< tos val minimum */
	union { uint8_t tos_min; uint8_t traffic_class_min; };
	/**< tos val maximum */
	union { uint8_t tos_max; uint8_t traffic_class_max; };
	...
	/**< TTL starting */
	union { uint8_t ttl; uint8_t hop_limits; };
	/**< TTL minimum */
	union { uint8_t ttl_min; uint8_t hop_limits_min; };
	/**< TTL maximum */
	union { uint8_t ttl_max; uint8_t hop_limits_max; };
