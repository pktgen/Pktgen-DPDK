...
pktgen_tcp_hdr_ctor(pkt_seq_t *pkt, void * hdr, int type)
{
	...
	if (type == __ETHER_TYPE_IPv4) {
		struct __ipv4_hdr *ipv4 = (struct __ipv4_hdr *)hdr;
		struct __tcp_hdr *tcp = (struct __tcp_hdr *)&ipv4[1];
		...
		tcp->data_off = ((sizeof(struct __tcp_hdr) / sizeof(uint32_t)) << 4);	/* Offset in words */
		...
		struct __ipv6_hdr *ipv6 = (struct __ipv6_hdr *)hdr;
		struct __tcp_hdr *tcp = (struct __tcp_hdr *)&ipv6[1];
		...
		tcp->recv_ack = htonl(DEFAULT_ACK_NUMBER);
		tcp->data_off =
			((sizeof(struct __tcp_hdr) / sizeof(uint32_t)) << 4);   /* Offset in words */
		tcp->tcp_flags = ACK_FLAG; /* ACK */
		tcp->rx_win     = htons(DEFAULT_WND_SIZE);
		tcp->tcp_urp = 0;
