...
extern int _unparse_frame(fgen_t *fg, frame_t *f);
...
static int
_unparse_opts(char *str, char **toks, int nb_toks)
{
    int len;
...
static int
_unparse_vars(char *str, char **vars, int nb_vars)
{
    int len, cnt;
...
static inline int
unparser_kvp(char *param, const char **kvps, int len, char **val)
{
    char *kvp[FGEN_MAX_KVP_TOKENS] = {0};

    if (_unparse_vars(param, kvp, RTE_DIM(kvp)) > 0) {
        for (int i = 0; i < len; i++) {
...
static int
_unparse_ether(fgen_t *fg, frame_t *f, int lidx)
{
    ...
    num = _unparse_opts(opt->param_str, fg->params, RTE_DIM(fg->params));
    if (num < 0)
        _ERR_RET("Parameters '%s' invalid\n", opt->param_str);

    for (int i = 0; i < num; i++) {
        switch (unparser_kvp(fg->params[i], kvps, RTE_DIM(kvps), &val)) {
        case 0:
    ...
    if (fg->flags & FGEN_VERBOSE)
        _INFO("[magenta]Return '[orange]%s[]'\n", unparser_type(opt->typ));

    return FGEN_ETHER_TYPE;
}

static int
_unparse_vlan(fgen_t *fg, frame_t *f, int lidx, bool is_dot1ad)
{
    fopt_t *opt = FGEN_LOPT(fg, lidx);
...
static int
_unparse_dot1q(fgen_t *fg, frame_t *f, int lidx)
{
    return _unparse_vlan(fg, f, lidx, false);
}

static int
_unparse_dot1ad(fgen_t *fg, frame_t *f, int lidx)
{
    return _unparse_vlan(fg, f, lidx, true);
}

static int
_unparse_ipv4(fgen_t *fg, frame_t *f, int lidx)
{
    fopt_t *opt = FGEN_LOPT(fg, lidx);
...
static int
_unparse_ipv6(fgen_t *fg, frame_t *f, int lidx)
{
    fopt_t *opt = FGEN_LOPT(fg, lidx);
...
static int
_unparse_udp(fgen_t *fg, frame_t *f, int lidx)
{
    fopt_t *opt = FGEN_LOPT(fg, lidx);
...
static int
_unparse_tcp(fgen_t *fg, frame_t *f, int lidx)
{
    fopt_t *opt = FGEN_LOPT(fg, lidx);
...
static int
_unparse_vxlan(fgen_t *fg, frame_t *f, int lidx)
{
    fopt_t *opt = FGEN_LOPT(fg, lidx);
...
static int
_unparse_echo(fgen_t *fg, frame_t *f, int lidx)
{
    fopt_t *opt = FGEN_LOPT(fg, lidx);
...
static int
_unparse_tsc(fgen_t *fg, frame_t *f, int lidx)
{
    fopt_t *opt = FGEN_LOPT(fg, lidx);
    char *tsc;
...
static int
_unparse_raw(fgen_t *fg, frame_t *f, int lidx)
{
    fopt_t *opt = FGEN_LOPT(fg, lidx);
...
static int
_unparse_payload(fgen_t *fg, frame_t *f, int lidx)
{
    fopt_t *opt = FGEN_LOPT(fg, lidx);
    int plen, num, fsize;
...
static int
_unparse_done(fgen_t *fg, frame_t *f, int lidx __rte_unused)
{
    fopt_t *opt = FGEN_LOPT(fg, lidx);
    ...
    if (fg->flags & FGEN_VERBOSE)
        _INFO("[magenta]Return '[orange]%s[]' [magenta]pktlen [orange]%d[]\n",
              unparser_type(opt->typ), fgen_data_len(f));
...
// clang-format off
ftable_t fgen_tbl[] = {
    {.str = "",         .fn = _unparse_done,      .typ = FGEN_DONE_TYPE},
    {.str = "Ether(",   .fn = _unparse_ether,     .typ = FGEN_ETHER_TYPE},
    {.str = "Dot1q(",   .fn = _unparse_dot1q,     .typ = FGEN_DOT1Q_TYPE},
    {.str = "Dot1ad(",  .fn = _unparse_dot1ad,    .typ = FGEN_DOT1AD_TYPE},

    {.str = "IPv4(",    .fn = _unparse_ipv4,      .typ = FGEN_IPV4_TYPE},
    {.str = "IPv6(",    .fn = _unparse_ipv6,      .typ = FGEN_IPV6_TYPE},

    {.str = "UDP(",     .fn = _unparse_udp,       .typ = FGEN_UDP_TYPE},
    {.str = "TCP(",     .fn = _unparse_tcp,       .typ = FGEN_TCP_TYPE},

    {.str = "Vxlan(",   .fn = _unparse_vxlan,     .typ = FGEN_VXLAN_TYPE},
    {.str = "Echo(",    .fn = _unparse_echo,      .typ = FGEN_ECHO_TYPE},
    {.str = "TSC(",     .fn = _unparse_tsc,       .typ = FGEN_TSC_TYPE},
    {.str = "Raw(",     .fn = _unparse_raw,       .typ = FGEN_RAW_TYPE},

    {.str = "Payload(", .fn = _unparse_payload,   .typ = FGEN_PAYLOAD_TYPE},
    {.str = NULL, .fn = NULL}
};
// clang-format on

int
_unparse_frame(fgen_t *fg, frame_t *f)
{
    fopt_t *opt = NULL;
    char *text;
