...
void
lua_newlibs_init(struct lua_Shell *ls) {
	lua_State *L = ls->L;
	int		i;
...
lua_shell(void * pServer)
{
	...
	ls = malloc(sizeof(struct lua_Shell));
	if (!ls) {
		lua_close(L);
		return EXIT_FAILURE;
	}
	memset(ls, 0, sizeof(struct lua_Shell));

	ls->L = L;
	lua_setprivate(ls, pServer);
...
int
execute_lua_string(struct lua_Shell *ls, char * buffer )
{
	lua_State *L = ls->L;
...
void
execute_lua_close(struct lua_Shell * ls)
{
	if ( ls->L )
		lua_close(ls->L);
}
...
LUA_API void
lua_setprivate(struct lua_Shell * ls, void * val) {
	ls->private = val;
}

LUA_API void *
lua_getprivate(struct lua_Shell * ls) {
	return ls->private;
}

void
lua_set_stdfiles(struct lua_Shell * ls, void * in, void * out, void * err)
{
	lua_State *L = ls->L;
	...
	create_stdfile(ls, in, IO_INPUT, "stdin");
	create_stdfile(ls, out, IO_OUTPUT, "stdout");
	create_stdfile(ls, err, NULL, "stderr");
}

void
lua_reset_stdfiles(struct lua_Shell * ls)
{
	lua_State *L = ls->L;
	...
	/* create (and set) default files */
	create_stdfile(ls, stdin, IO_INPUT, "stdin");
	create_stdfile(ls, stdout, IO_OUTPUT, "stdout");
	create_stdfile(ls, stderr, NULL, "stderr");
}
...
void
create_stdfile (struct lua_Shell *ls, FILE *f, const char *k, const char *fname)
{
	lua_State *L = ls->L;
