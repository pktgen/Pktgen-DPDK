...
--
--  Improved 2x100GBE version v1.0 - Contributed by Niclas Edin
--  Set 4 CPU cores per Rx:Tx function for +200MPPS performance, tested using 2xC810 on Intel(R) Xeon(R) Gold 6338N
...
require "Pktgen";
local DEBUG		= true;
local USE_RAMPUP	= false; 	--Rampup traffic flow before starting measurement, less accurate time measurement.
...
-- Set max rate profile for PktGen to avoid crashing some interface queues during overload (XXV710, C810)
local pktgen_limit	= {
  				[64]   = { ["rate"] = 75 },
  				[128]  = { ["rate"] = 100 },
  				[256]  = { ["rate"] = 100 },
...
local loss_tol        = 0.10;
local seekPrec		    = 0.15;		-- Divide n conquer seek precision
...
-- Flows ~ num_ip * num_udpp * 4. Max 10M flows, only managed to get 200k active flows.
...
--local p0_destmac	  = "b4:96:91:b4:00:11";	-- Comment out to test Pktgen loopback
--local p1_destmac	  = "b4:96:91:b4:00:12";	--
...
local function runTrial(pkt_size, rate, duration, count, retry)
	...
	-- Check that port Tx opackets are within reason, otherwise reset interface being low, and reload IP config.
	-- TODO: Verify that reset does recover interfaces misbehaving. Is there a link down function?
	...
	then
		print("[WARNING] Port:"..sendport.." Tx underruns. Resetting ports. Results might be inconclusive.("..Round(num_tx - num_tx1)..")");
    pktgen.reset("all");
    pktgen.reset("all");
		pktgen.delay(500);
    pktgen.reset("all");
	...
	then
		print("[WARNING] Port:"..recvport.." Tx underruns. Resetting ports. Results might be inconclusive.("..Round(num_tx1 - num_tx)..")");
    pktgen.reset("all");
		pktgen.reset("all");
		pktgen.delay(500);
    pktgen.reset("all");
